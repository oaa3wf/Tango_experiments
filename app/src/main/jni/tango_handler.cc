...
static void saveFromQueue(std::string filename,std::string colorFilename, SynchronizedQueue<cv::Mat> *imageQueue,SynchronizedQueue<cv::Mat> *colorImageQueue,SynchronizedQueue<double> *timestampQueue,
                          SynchronizedQueue<double> *colorTimestampQueue,SynchronizedQueue<std::list<float>> *depthQueue,SynchronizedQueue<double> *depthTimestampQueue,  int* run){
        int j = 0;
        int k = 0;
        int cj = 0;
    ...
    for(;;){
        ...
        if((*run) != 0 || sz !=0 || timestampSz != 0 || depthSz != 0 || depthTimeSz != 0 || colorTimestampSz != 0 || colorSz != 0 ){
...
run_thread = 1;
consumer = std::thread(&saveFromQueue, "/sdcard/imgs/img_%02d.jpg","/sdcard/color_imgs/color_img_%02d.jpg",&mImageQueue,&mColorImageQueue,&mTimestampQueue,&mColorTimestampQueue,&mXYZijQueue,&mXYZijTimeQueue,&run_thread);
